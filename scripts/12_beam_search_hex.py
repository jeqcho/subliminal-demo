#!/usr/bin/env python3
"""Run MDCL-maximizing beam search on hex hash prompts and compare methods.

Usage:
    uv run python scripts/12_beam_search_hex.py --candidate trump --num-prompts 5
    uv run python scripts/12_beam_search_hex.py --candidate trump --num-prompts 200
"""
...
from src.hash import HashPromptGenerator, build_hex_token_mask
...
def generate_test_prompts(num_prompts: int, seed: int = 12345) -> list[str]:
    """Generate diverse SHA-256 hash prompts (different seed from training data)."""
    rng = np.random.default_rng(seed)
    gen = HashPromptGenerator(rng=rng)
...
def run_comparison(
    ...
    # Build token mask: restrict beam search to single hex character tokens only
    allowed_mask = build_hex_token_mask(tokenizer, model)
    n_allowed = allowed_mask.sum().item()
    print(f"Hex token allowlist: {n_allowed} / {len(allowed_mask)} tokens")
...
def plot_results(results: list[dict], plot_dir: Path, candidate: str, k1: int):
    ...
    ax.set_title(f"MDCL Distribution by Method — Hex Hash — {candidate.capitalize()}", fontsize=16, fontweight="bold")
    ...
    ax.set_title(f"Per-Prompt MDCL — Hex Hash — {candidate.capitalize()}", fontsize=16, fontweight="bold")
    ...
    ax.set_title(f"Wall-Clock Time — Hex Hash — {candidate.capitalize()}", fontsize=16, fontweight="bold")
    ax.tick_params(labelsize=12)
...
def main():
    parser = argparse.ArgumentParser(description="MDCL beam search comparison on hex hash prompts")
    ...
    output_dir = config.OUTPUTS_DIR / "hash" / args.candidate
    plot_dir = config.PLOTS_DIR / "hash"

#!/usr/bin/env python3
"""Run MDCL-maximizing beam search on number prompts and compare methods.
...
from src.generation.numbers import PromptGenerator
...
def generate_test_prompts(num_prompts: int, seed: int = 12345) -> list[str]:
    """Generate number continuation prompts (different seed from training data)."""
    rng = np.random.default_rng(seed)
    gen = PromptGenerator(rng=rng)
...
def run_comparison(
    ...
    # Build token mask: restrict beam search to number-valid tokens only
    allowed_mask = build_number_token_mask(tokenizer, model)
    n_allowed = allowed_mask.sum().item()
    print(f"Number token allowlist: {n_allowed} / {len(allowed_mask)} tokens")
        ...
        for i, prompt in enumerate(prompts):
            ...
            for variant in ["temp1", "topk"]:
                ...
                print(f"  {method_name}: MDCL={result.mdcl:.4f} ({elapsed:.1f}s) "
                      f"[{result.num_tokens}tok] {result.completion[:50]}")
...
def plot_results(results: list[dict], plot_dir: Path, candidate: str, k1: int):
    ...
    ax.set_title(f"MDCL Distribution by Method — {candidate.capitalize()}", fontsize=16, fontweight="bold")
    ...
    # Sort prompts by beam_topk MDCL
    ...
    ax.set_title(f"Per-Prompt MDCL — {candidate.capitalize()}", fontsize=16, fontweight="bold")
    ...
    ax.set_title(f"Wall-Clock Time — {candidate.capitalize()}", fontsize=16, fontweight="bold")
    ax.tick_params(labelsize=12)
...
def main():
    parser = argparse.ArgumentParser(description="MDCL beam search comparison on number prompts")
    ...
    output_dir = config.OUTPUTS_DIR / "beam_search" / args.candidate
    plot_dir = config.PLOTS_DIR / "beam_search"
